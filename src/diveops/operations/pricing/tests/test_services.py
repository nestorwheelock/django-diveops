...
class TestCreateLedgerEntriesIntegration:
    ...
    def test_ledger_transaction_has_balanced_entries(self, mock_booking):
        ...
        with patch("diveops.operations.pricing.services.ContentType") as mock_ct:
            with patch("diveops.operations.pricing.services.Account") as mock_account:
                with patch("diveops.operations.pricing.services.Transaction") as mock_tx:
                    with patch("diveops.operations.pricing.services.Entry") as mock_entry:
                        # Setup mocks
...
class TestSettlementBatchSavepoints:
    ...
    def test_savepoint_allows_partial_success(self):
        ...
        from diveops.operations.settlement_service import run_settlement_batch
        import inspect
